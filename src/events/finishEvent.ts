...
async function getEventPool(IdEvt: number) {
...
async function getWinnerGamblers(IdEvt: number,ResultadoEvento: string){
    OracleDB.outFormat = OracleDB.OUT_FORMAT_OBJECT;
    let connection;
    ...
    try{
        ...
        console.log('DataBase error',err);
        throw new Error("Error during geting event WinnerPool.");
    }finally{
        if (connection){
            try{
                await connection.close();
...
async function distributePool(IdEvt: number,ResultadoEvento: string,pool: number,winnersPool: number) {
    OracleDB.outFormat = OracleDB.OUT_FORMAT_OBJECT;
    let connection;
    ...
    try{
        ...
        console.log('DataBase error',err);
        throw new Error("Error during distributing pool.");
    }finally{
        if (connection){
            try{
                await connection.close();
